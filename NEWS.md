...
* `parallel_merge()` split workers now create tile-local clustering shards via
  ...
  algorithm remains full-size. `clust_slink()` leaf instances allocate SLINK
  pointer arrays; merge parents cache child MST edges and replay them through
  SLINK at finalize (no delegate tree). With one thread, merge matches
  concurrent and the root holds only the SLINK arrays.
* The clustering memory estimator for `parallel_merge()` now accounts for
  ...
  thread) use a smaller coefficient; multi-thread merge parents use
  `SLINK + 16 n T` for the merge-cache peak. Optional `include_result`
  adds the `n x m` integer output for tree/SLINK.
