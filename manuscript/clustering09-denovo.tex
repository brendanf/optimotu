...
%  15.681818/269.15909/g1/g1s2/\spradA/\known,%
  ...
  103/255/g2/g2s1/\spradB/\refsecond,%
...
\foreach \x / \y / \genus / \species / \radius / \type [count = \i] in \asvs {
  \begin{scope}
    ...
    % \ifnum\pgfmathresult=1
      ...
      \draw[color=\species, line width=0.6mm] (\x, \y) circle (\radius);
    % \fi
  \end{scope}}
...
%     \relax
%   \else
%     \draw[dashed, line width=0.3mm] (\x, \y) circle (\radius);
%   \fi
% }
  ...
  \begin{scope}
    \foreach \xb / \yb / \genusb / \speciesb / \radiusb / \typeb [count = \j] in \asvs {
      \ifnum\i>\j
        ...
        \ifnum\pgfmathresult=1
          \ifx\species\speciesb
            \pgfmathparse{(\type==\known && \typeb==\reffirst) || (\type==\reffirst && \typeb==\known) ? 1 : 0}
            \ifnum\pgfmathresult=1
              \draw[\species, reffirst] (\x, \y) -- (\xb, \yb);
            \else
              \pgfmathparse{(\type==\reffirst && \typeb==\refsecond) || (\type==\refsecond && \typeb==\reffirst) ? 1 : 0}
              \ifnum\pgfmathresult=1
                \draw[\species, refsecond] (\x, \y) -- (\xb, \yb);
              \else
                \pgfmathparse{(\type==\denovo && \typeb==\denovo) ? 1 : 0}
                \ifnum\pgfmathresult=1
...
%                   \fi
            ...
            \fi
          \else
            \pgfmathparse{\type==\known && \typeb==\known ? 1 : 0}
            \ifnum\pgfmathresult=1
...
\foreach \x / \y / \genus / \species / \radius / \type in \asvs {
  ...
  \else
    \draw[unknown, fill=white] (\x, \y) circle (\asvrad);
  \fi
}
...
\draw [reffirst, red] (158,198) -- (173,198) node[pos=0.5,sloped] {\sffamily\bfseries{X}}
node[color = black, anchor=west] at ++(0, 2) {Illegal linkage};
...
\draw [g2s1!50!black, fill=g2s1] (171,190) circle (\asvrad) node[right=2mm, color = black] {Seq. identified to species};
\draw [unknown, fill=white] (171,180) circle (\asvrad) node[right=2mm, color = black] {Unidentified seq.};
