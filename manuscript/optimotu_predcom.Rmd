...
1. \alert<3>{Use optimized clustering thresholds for each taxon.}
...
## Deleted scene: Optimizing clustering thresholds
...
## Deleted scene: Taxonomic assignment

- **OptimOTU R package** leaves this to the user
- Can use any tool with "stopping condition"
...
- Searching for the best match with BLAST, VSEARCH, or similar (1-NN) has no stopping condition.
- Many existing AI methods (e.g., BarcodeBERT) have no stopping condition.

- **OptimOTU pipeline** uses PROTAX with two different thresholds:
...
```{r out.width="100%"}
knitr::include_graphics("PredCom_Oikos_2025.jpeg")
```
