    ...
    const PairGenerator &pg)
{
  ...
  locals.reserve(global_to_subset.size());
  for (const auto &kv : global_to_subset)
  {
  ...
  std::vector<std::size_t> intersection_fwd;
  intersection_fwd.reserve(locals.size());
  for (j_t local : locals)
  {
    intersection_fwd.push_back(static_cast<std::size_t>(local));
  }
  return intersection_fwd;
}

namespace
{

  std::size_t rev_map_lookup(
      const std::unordered_map<std::size_t, std::size_t> &rev_map,
      j_t external,
      const char *context)
  {
    auto it = rev_map.find(static_cast<std::size_t>(external));
    if (it == rev_map.end())
    {
      OPTIMOTU_STOP(
          std::string(context) + ": parent index " + std::to_string(external) +
          " not in mapped shard (child_n=" +
          std::to_string(rev_map.size()) + ")");
    }
    return it->second;
  }
...
MCA::MappedClusterAlgorithm(ClusterAlgorithm *parent, j_t n) : SingleClusterAlgorithm(parent, n)
{
}

MCA::MappedClusterAlgorithm(const DistanceConverter &dconv, j_t n) : SingleClusterAlgorithm(dconv, n)
{
}
...
MappedClusterAlgorithmImpl<verbose>::Surrogate::Surrogate(
    ...
    const std::vector<std::size_t> &fwd_map) : ClusterAlgorithm(parent),
                                               fwd_map(fwd_map)
{
}
...
void MappedClusterAlgorithmImpl<verbose>::Surrogate::operator()(j_t seq1, j_t seq2, double dist, int thread)
{
  ...
  {
    OPTIMOTU_STOP(
        "MappedClusterAlgorithm::Surrogate: inner index out of range for mapped merge");
  }
  const j_t p1 = static_cast<j_t>(fwd_map[seq1]);
  const j_t p2 = static_cast<j_t>(fwd_map[seq2]);
  auto *sp = dynamic_cast<SingleClusterAlgorithm *>(parent);
  if (sp && (p1 >= sp->n || p2 >= sp->n))
  {
    OPTIMOTU_STOP(
        "MappedClusterAlgorithm::Surrogate: mapped parent index out of range "
        "(p1=" +
        std::to_string(p1) + ", p2=" + std::to_string(p2) +
        ", n=" + std::to_string(sp->n) + ", inner seq1=" +
        std::to_string(seq1) + ", inner seq2=" + std::to_string(seq2) +
        ", map_size=" + std::to_string(fwd_map.size()) + ")");
  }
  (*parent)(p1, p2, dist, thread);
}
template <int verbose>
...
MappedClusterAlgorithmImpl<verbose>::DistanceForwarder::DistanceForwarder(
    const std::vector<std::size_t> &fwd_map,
    DistanceConsumer *consumer) : fwd_map(fwd_map),
                                  target(consumer)
{
}
template <int verbose>
...
void MappedClusterAlgorithmImpl<verbose>::DistanceForwarder::operator()(j_t seq1, j_t seq2, double dist, int thread)
{
  ...
  {
    OPTIMOTU_STOP(
        "MappedClusterAlgorithm::DistanceForwarder: inner index out of range");
  }
...
MappedClusterAlgorithmImpl<verbose>::IndexForwarder::IndexForwarder(
    const std::vector<std::size_t> &fwd_map,
    ClusterAlgorithm *target) : ClusterAlgorithm(target),
                                fwd_map(fwd_map),
                                target(target)
{
}
...
void MappedClusterAlgorithmImpl<verbose>::IndexForwarder::operator()(j_t seq1, j_t seq2, double dist, int thread)
{
  ...
  {
    OPTIMOTU_STOP(
        "MappedClusterAlgorithm::IndexForwarder: inner index out of range");
  }
  (*target)(fwd_map[seq1], fwd_map[seq2], dist, thread);
}
template <int verbose>
...
template <int verbose>
MappedClusterAlgorithmImpl<verbose>::MappedClusterAlgorithmImpl(SingleClusterAlgorithm *parent, PairGenerator *pg) : MappedClusterAlgorithm(parent, pg->max_value() + 1),
...
MappedClusterAlgorithmImpl<verbose>::MappedClusterAlgorithmImpl(
    ...
    PairGenerator *pg) : MappedClusterAlgorithm(parent, pg->max_value() + 1),
                         fwd_map(map_from_pair_generator(*pg)),
                         rev_map(invert_map(fwd_map)),
...
MappedClusterAlgorithmImpl<verbose>::MappedClusterAlgorithmImpl(
    SingleClusterAlgorithm *parent,
    SingleClusterAlgorithm *delegate,
    const std::vector<std::size_t> &intersection_fwd,
    const std::unordered_map<std::size_t, std::size_t> &intersection_rev) : MappedClusterAlgorithm(parent, intersection_fwd.size()),
                                                                            fwd_map(intersection_fwd),
                                                                            rev_map(intersection_rev),
                                                                            surrogate(std::make_unique<Surrogate>(delegate, this->fwd_map)),
                                                                            inner(parent->make_inner_child(surrogate.get(), intersection_fwd.size())),
...
MappedClusterAlgorithmImpl<verbose>::MappedClusterAlgorithmImpl(
    SingleClusterAlgorithm *parent,
    const std::vector<std::size_t> &intersection_fwd,
    const std::unordered_map<std::size_t, std::size_t> &intersection_rev) : MappedClusterAlgorithm(parent->dconv, intersection_fwd.size()),
                                                                            fwd_map(intersection_fwd),
                                                                            rev_map(intersection_rev),
                                                                            surrogate(std::make_unique<Surrogate>(parent, this->fwd_map)),
                                                                            inner(parent->make_inner_child(surrogate.get(), intersection_fwd.size())),
                                                                            pair_generator(nullptr)
{
}
...
template <int verbose>
MappedClusterAlgorithmImpl<verbose>::MappedClusterAlgorithmImpl(
    SingleClusterAlgorithm &inner_ref, const std::vector<std::size_t> &fwd_map_in) : MappedClusterAlgorithm(inner_ref.dconv, inner_ref.n),
                                                                                     fwd_map(fwd_map_in),
                                                                                     rev_map(invert_map(fwd_map)),
                                                                                     surrogate(),
                                                                                     inner(&inner_ref),
...
{
  (*inner)(
      rev_map_lookup(rev_map, seq1, "MappedClusterAlgorithm::operator()"),
      rev_map_lookup(rev_map, seq2, "MappedClusterAlgorithm::operator()"),
      dist,
      thread);
}
template <int verbose>
...
void MappedClusterAlgorithmImpl<verbose>::operator()(PairGenerator &pg, double dist, int thread)
{
  ...
  {
    (*inner)(
        rev_map_lookup(rev_map, pg.i0(), "MappedClusterAlgorithm::operator()"),
        rev_map_lookup(rev_map, pg.j0(), "MappedClusterAlgorithm::operator()"),
        dist,
        thread);
  }
}
...
MappedClusterAlgorithm *MappedClusterAlgorithmImpl<verbose>::make_child(PairGenerator *pg)
{
  ...
  {
    new_fwd_map[k] = rev_map_lookup(
        rev_map,
        static_cast<j_t>(new_fwd_map[k]),
        "MappedClusterAlgorithm::make_child(pg)");
  }
  std::unique_lock<std::shared_timed_mutex> lock(this->mutex);
...
{
  return inner->max_relevant(
      rev_map_lookup(rev_map, seq1, "MappedClusterAlgorithm::max_relevant"),
      rev_map_lookup(rev_map, seq2, "MappedClusterAlgorithm::max_relevant"),
      thread);
}
...
double MappedClusterAlgorithmImpl<verbose>::max_relevant(PairGenerator &pg, int thread) const
{
  ...
  {
    return inner->max_relevant(
        rev_map_lookup(rev_map, pg.i0(), "MappedClusterAlgorithm::max_relevant"),
        rev_map_lookup(rev_map, pg.j0(), "MappedClusterAlgorithm::max_relevant"),
        thread);
  }
}
...
context("MappedClusterAlgorithm intersection merge")
{

  test_that("fwd_map shard merges into parent with valid indices")
  {
    UniformDistanceConverter dconv(0.0, 0.4, 0.01);
    ClusterTreeImpl<0, 0> parent(nullptr, 10);
    std::vector<std::size_t> intersection_fwd{0, 2, 4, 6, 8};
    ClusterAlgorithm *shard = parent.make_child(intersection_fwd);
    expect_true(shard != nullptr);
    (*shard)(0, 2, 0.05);
    (*shard)(4, 6, 0.05);
    shard->merge_into_parent();
    parent.release_child(shard);
  }
}
#endif