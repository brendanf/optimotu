...
void Ksw2DistWorkerImpl<verbose, is_constrained, span, SparseDistanceMatrixType>::operator()(std::size_t begin, std::size_t end) {
  double n = seq.size();
  ...
  for (size_t i = begin_i; i < end_i; i++) {
    for (size_t j = 0; j < i; j++) {
      ...
      if constexpr (is_constrained) {
        ...
        aligner.setBandWidth(-1);
      }

      // Longer as query, shorter as target
      std::string cigar = cigar_ksw2<span>(seq[s2], seq[s1], aligner);
      if (cigar == "") continue;
      ++my_aligned;
      ...
      if constexpr (std::is_same_v<SparseDistanceMatrixType, SparseDistanceMatrixCigar>) {
        ...
        auto [d2, aln_len, n_ins, n_del, max_ins, max_del] = dist_gapstats_from_cigar(cigar);
        (void)d2;
...
std::unique_ptr<Ksw2DistWorker> create_ksw2_dist_worker(
  ...
  if (dynamic_cast<SparseDistanceMatrixCigar*>(&sdm)) {
    return std::make_unique<Ksw2DistWorkerImpl<verbose, is_constrained, span, SparseDistanceMatrixCigar>>(seq, dist_threshold, pgb, sdm, match, mismatch, gap_open, gap_extend, gap_open2, gap_extend2);
  } else if (dynamic_cast<SparseDistanceMatrixGapstats*>(&sdm)) {
    return std::make_unique<Ksw2DistWorkerImpl<verbose, is_constrained, span, SparseDistanceMatrixGapstats>>(seq, dist_threshold, pgb, sdm, match, mismatch, gap_open, gap_extend, gap_open2, gap_extend2);
  } else {
    return std::make_unique<Ksw2DistWorkerImpl<verbose, is_constrained, span, SparseDistanceMatrix>>(seq, dist_threshold, pgb, sdm, match, mismatch, gap_open, gap_extend, gap_open2, gap_extend2);
  }
}
...
std::unique_ptr<Ksw2DistWorker> create_ksw2_dist_worker(
  ...
  switch (span) {
    case AlignmentSpan::GLOBAL:
      return create_ksw2_dist_worker<verbose, is_constrained, AlignmentSpan::GLOBAL>(seq, dist_threshold, pgb, sdm, match, mismatch, gap_open, gap_extend, gap_open2, gap_extend2);
    case AlignmentSpan::EXTEND:
      return create_ksw2_dist_worker<verbose, is_constrained, AlignmentSpan::EXTEND>(seq, dist_threshold, pgb, sdm, match, mismatch, gap_open, gap_extend, gap_open2, gap_extend2);
    default:
...
std::unique_ptr<Ksw2DistWorker> create_ksw2_dist_worker(
  ...
  if (is_constrained) {
    return create_ksw2_dist_worker<verbose, true>(seq, dist_threshold, pgb, sdm, match, mismatch, gap_open, gap_extend, gap_open2, gap_extend2, span);
  } else {
    return create_ksw2_dist_worker<verbose, false>(seq, dist_threshold, pgb, sdm, match, mismatch, gap_open, gap_extend, gap_open2, gap_extend2, span);
  }
}
...
std::unique_ptr<Ksw2DistWorker> create_ksw2_dist_worker(
  ...
  switch (v) {
    case 0:
      return create_ksw2_dist_worker<0>(seq, dist_threshold, pgb, sdm, match, mismatch, gap_open, gap_extend, gap_open2, gap_extend2, span, constrain);
    case 1:
      return create_ksw2_dist_worker<1>(seq, dist_threshold, pgb, sdm, match, mismatch, gap_open, gap_extend, gap_open2, gap_extend2, span, constrain);
    case 2:
      return create_ksw2_dist_worker<2>(seq, dist_threshold, pgb, sdm, match, mismatch, gap_open, gap_extend, gap_open2, gap_extend2, span, constrain);
    case 3:
      return create_ksw2_dist_worker<3>(seq, dist_threshold, pgb, sdm, match, mismatch, gap_open, gap_extend, gap_open2, gap_extend2, span, constrain);
    case 4:
    default:
      return create_ksw2_dist_worker<4>(seq, dist_threshold, pgb, sdm, match, mismatch, gap_open, gap_extend, gap_open2, gap_extend2, span, constrain);
  }
}
