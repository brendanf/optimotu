...
struct ConfusionMatrixWorker2 : RcppParallel::Worker {
  const RcppParallel::RMatrix<int> k;
  const RcppParallel::RMatrix<int> c;
  RcppParallel::RVector<double> tp, fp, fn, tn;
  const size_t N, Npairs;

  ConfusionMatrixWorker2(
    Rcpp::IntegerMatrix k,
    ...
    Rcpp::NumericVector fn,
    Rcpp::NumericVector tn
  ) : k(k), c(c), tp(tp), fp(fp), fn(fn), tn(tn),
  N(c.ncol()), Npairs(N * (N-1) / 2) {};
  ...
  void operator()(size_t begin, size_t end) {
    ...
    for (std::size_t j = begin; j < end; ++j) {
      ...
      c_sort.clear();
      c_count.clear();
      initialize_counts(cj, c_sort, c_count, N);
      tpj = 0;
      fpj = 0;
      fnj = 0;
      k_count.clear();
...
Rcpp::DataFrame confusion_matrix2(
    const Rcpp::IntegerMatrix k,
    const Rcpp::IntegerMatrix c,
    const int threads = 1
) {
  size_t N = c.ncol(), m = k.nrow();
  if (N != (size_t)k.ncol())
    OPTIMOTU_STOP("test clusters 'k' (%d) and true clusters 'c' (%d) must have"
                    " the same number of objects.", k.ncol(), N);
  if (m != (size_t)c.nrow())
    OPTIMOTU_STOP("test clusters 'k' (%d) and true clusters 'c' (%d) must have"
                    " the same number of thresholds.", m, c.nrow());
  Rcpp::NumericVector true_positive(m);
  Rcpp::NumericVector false_positive(m);
  Rcpp::NumericVector false_negative(m);
  Rcpp::NumericVector true_negative(m);

  ConfusionMatrixWorker2 worker(k, c,
                               true_positive, false_positive,
                               false_negative, true_negative);
  if (threads == 1) {
    worker(0, m);
